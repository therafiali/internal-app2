...
export default function AssignDepositRequestDialog({
    ...
    const [redeemWithPaymentMethods, setRedeemWithPaymentMethods] = useState<RedeemWithPaymentMethods[]>([]);
    const { user } = useAuth();
                ...
                redeemRequests.map(async (redeem) => {
                    try {
                        ...
                        return {
                            ...
                            redeemAvailable: redeem.amount_available || 0,
                            redeemPaymentMethod: redeem.payment_methods?.payment_method,
                            playerPaymentMethods: playerPaymentMethods as PlayerPaymentMethod[],
                            id: redeem.id
                        };
                    } catch (error) {
    ...
    const filteredRedeemTableData = redeemWithPaymentMethods?.filter((redeem) => {
        ...
        const matchesRedeemPaymentMethod = redeemPaymentMethod?.toLowerCase() === selectedPaymentMethod?.toLowerCase();

        // Also check if any of the player's payment methods match the selected payment method
        const matchesPlayerPaymentMethod = redeem.playerPaymentMethods?.some(method =>
            method.payment_methods?.payment_method?.toLowerCase() === selectedPaymentMethod?.toLowerCase()
        );
        ...
        console.log("Filtering redeem:", {
            ...
            redeemPaymentMethod,
            playerPaymentMethods: redeem.playerPaymentMethods?.map(m => m.payment_methods?.payment_method),
            matchesRedeemPaymentMethod,
            matchesPlayerPaymentMethod,
            shouldShow: matchesRedeemPaymentMethod || matchesPlayerPaymentMethod
        });

        // Show if either the redeem payment method or any player payment method matches
        return matchesRedeemPaymentMethod || matchesPlayerPaymentMethod;
    });
    ...
    const handleRedeemAssign = async (redeemId: string) => {
        ...
        try {
            ...
            toast.success(`Successfully assigned recharge ${selectedRow.id} to redeem ${redeemId}`);
            ...
            toast.error(`Failed to assign redeem: ${error instanceof Error ? error.message : 'Unknown error'}`);
        } finally {
            setLoading(false);
                        ...
                        <div className="bg-gray-800 rounded-lg overflow-hidden">
                            {redeemRequestsLoading ? (
                                <div className="p-8 text-center">
                                    <div className="text-gray-400">Loading redeem requests...</div>
                                </div>
                            ) : redeemRequestsError ? (
                                <div className="p-8 text-center">
                                    <div className="text-red-400">Error loading redeem requests: {redeemRequestsError.message}</div>
                                </div>
                            ) : !filteredRedeemTableData || filteredRedeemTableData.length === 0 ? (
                                <div className="p-8 text-center">
                                    <div className="text-gray-400">No redeem requests available</div>
                                </div>
                            ) : (
                                                ...
                                                <tr key={redeem.redeemId} className="hover:bg-gray-700">
                                                    ...
                                                    <td className="px-4 py-3 text-sm text-gray-300">
                                                        <div className="space-y-1">
                                                            ...
                                                            {redeem.playerPaymentMethods && redeem.playerPaymentMethods.length > 0 ? (
                                                                <div className="text-xs">
                                                                    {redeem.playerPaymentMethods.map((method, index) => (
                                                                        <div key={method.id} className="text-gray-400">
                                                                            {method.payment_methods?.payment_method || 'Unknown'}
                                                                            {method.tag_name && ` (${method.tag_name})`}
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            ) : (
                                                                <div className="text-red-400 text-xs">No payment methods found</div>
                                                            )}
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-sm">
                                                        <Button
                                                            size="sm"
                                                            onClick={() => handleRedeemAssign(redeem.redeemId)}
                                                            disabled={loading}
