...
import {
  useFetchPlayer,
  useFetchPlayerByTeam,
} from "~/hooks/api/queries/useFetchPlayer";
import { useLoaderData } from "@remix-run/react";
import { useFetchGameUsernames } from "~/hooks/api/queries/useFetchGames";
import {
  useSubmitRedeemRequest,
  type RedeemRequestData,
} from "~/hooks/api/mutation/submit-redeem";
...
export function SubmitRedeemModal({
  ...
  const { data: playerPaymentMethods, refetch: refetchPlayerPaymentMethods } =
    useFetchPlayerPaymentMethodDetail(selectedPlayer?.id || "");
  const { data: gameUsernames } = useFetchGameUsernames(
    selectedPlayer?.id || ""
  );
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to submit redeem request. Please try again.";
                  ...
                  <div className="space-y-2">
                    {gameUsernames.data.map((game) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setSelectedUsername(game.id || "");
                          setSelectedPlatform(game.games.id || "");
                        }}
                        className={`w-full flex items-center justify-between p-3 rounded-lg border-2 transition-all ${
                          selectedUsername === (game.id || "")
                      ...
                      <button
                        ...
                        onClick={() =>
                          handlePaymentMethodSelect(
                            playerMethod.payment_method,
                            playerMethod.payment_methods?.id || ""
                          )
                        }
                        className={`w-full flex items-center justify-between p-3 rounded-lg border-2 transition-all ${
                      ...
                      >
                        <div className="flex flex-col items-start">
                          <span className="text-white font-medium">
                            {playerMethod.payment_methods?.payment_method ||
                              "Unknown Method"}
                          </span>
                          {playerMethod.tag_name && (
                            <span className="text-gray-400 text-sm">
                              Tag: {playerMethod.tag_name}
                            </span>
                          )}
                        </div>
