...
export default function NewAccountModal({
  ...
  // Mock game platforms - replace with actual data from your API
  const platforms = [
    "Call of Duty",
    "Fortnite",
    "PUBG",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPlayer || !gamePlatform || !pageName) {
    ...
    try {
      ...
      if (error) {
        console.error("Error creating new account request:", error);
        alert("Failed to create new account request");
        return;
      }
      ...
      alert("New account request submitted successfully!");
    } catch (error) {
      console.error("Error submitting new account request:", error);
      alert("Failed to submit new account request");
    } finally {
      ...
      <DialogContent className="bg-[#181A20] border border-gray-700 text-white max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end">
            <Button
              type="submit"
              disabled={isSubmitting || !selectedPlayer || !gamePlatform || !pageName}
