...
export default function TransferRequestPage() {
  type RowType = {
    ...
    processedBy: string;
    duration: string;
    operation_transfer_process_status?: string;
    operation_transfer_process_by?: string;
  };
  ...
  const columns = [
    ...
    { accessorKey: "duration", header: "DURATION" },
    {
      ...
      cell: ({ row }: { row: { original: RowType } }) => (
        <Button
          disabled={
            row.original.operation_transfer_process_status === "in_process"
          }
          onClick={async () => {
            ...
            const { data: rowData } = await supabase
              ...
              .select(
                "operation_transfer_process_status, operation_transfer_process_by, users:operation_transfer_process_by (name, employee_code)"
              )
              .eq("id", row.original.id);
            console.log(rowData, "rowData");
            if (
              rowData &&
              rowData[0].operation_transfer_process_status === "in_process"
            ) {
              window.alert(
                rowData[0].operation_transfer_process_status +
                  " already in process" +
                  " by " +
                  rowData[0].operation_transfer_process_by
              );
              refetchData();
              return;
            }
            ...
            if (userData.user) {
              ...
              await supabase
                .from("transfer_requests")
                .update({
                  operation_transfer_process_status: "in_process",
                  operation_transfer_process_by: currentUserId,
                  operation_transfer_process_at: new Date().toISOString(),
                })
                .eq("id", row.original.id);
        ...
        >
          {row.original.operation_transfer_process_status === "in_process"
            ? `In Process${
                row.original.operation_transfer_process_by
  ...
  const tableData: RowType[] = (Array.isArray(data) ? data : []).map(
    ...
    (item: any) => {
      return {
        ...
        from: item.from_platform ?? "-",
        amount: item.amount ? `$${item.amount}` : "$0",
        to: item.to_platform ?? "-",
        status: item.status ?? "Pending",
        processedBy: item.processed_by ?? "-",
        duration: item.created_at ? new Date(item.created_at).toLocaleString() : "-",
        operation_transfer_process_status: item.operation_transfer_process_status,
        operation_transfer_process_by: item.operation_transfer_process_by,
      };
    }
  );
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="bg-gray-900 rounded-lg p-4 border border-gray-700">
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-gray-300">TRANSFER DETAILS</h3>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Transfer ID</p>
                    <p className="text-white font-medium font-mono bg-gray-800 px-2 py-1 rounded text-sm">
                      {selectedRow.transferId || "N/A"}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">From</p>
                    <p className="text-white font-medium">{selectedRow.from || "N/A"}</p>
                  </div>
                </div>
              </div>
